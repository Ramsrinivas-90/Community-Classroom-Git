...
where W.CURR_IND = '1' AND W.CRE_DTTM >= NOW() - INTERVAL 24 HOUR  and (STEP.LKP_CD_DESC="HITL" or STEP.LKP_CD_DESC="Data Delivered")  and (STS.LKP_CD_DESC="Success Notified" or STS.LKP_CD_DESC="Queued") AND I.CLIENT_TYPE_ID !=8) A group by WRKFLW_STEP,WRKFLW_STS;;
'''
...
where W.CURR_IND = '1' AND W.CRE_DTTM >= NOW() - INTERVAL 24 HOUR AND I.CLIENT_TYPE_ID !=8;'''

splitCount = '''SELECT DOC_TYPE,Count(DOC_TYPE) FROM ICEe.EDA_EXTRCTN_WRKFLW_DTL where BLOB_DTL_ID in (select I.BLOB_DTL_ID
from ICEe.EDA_EXTRCTN_WRKFLW_DTL W left join INGS_BLOB_DTL I on W.BLOB_DTL_ID = I.BLOB_DTL_ID
where W.CURR_IND = '1' AND W.CRE_DTTM >= NOW() - INTERVAL 24 HOUR AND WRKFLW_STS_TYPE_ID = '11' AND I.CLIENT_TYPE_ID !=8) and CURR_IND = '1' AND CRE_DTTM >= NOW() - INTERVAL 24 HOUR group by DOC_TYPE;'''
